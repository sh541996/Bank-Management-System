...
import com.cts.bms.controller.InvestmentController;
...
public class InvestmentServiceImpl implements InvestmentService {
	...
	// to save the detail of a investment in a db
	@Override
	public Investment saveInvestmentService(Investment investment) throws PanOrAccountInvalid, FundNameInvalid {
		...
		// to check is given fund name by user is available in db or not by checking in mutual_Fund table
		if(mutualFundList.stream().filter( item-> item.getFundName().equals(investment.getFundName().toUpperCase())).count()==1) {
			
			// to check weather the pan and account provided by user exist in db or not using account microservice
	...
	public List<Investment> summaryService(@Valid String pan) throws NoInvestment {
		...
		Collections.sort(listInvestment, (investment1, investment2)-> investment2.getFundName().compareTo(investment1.getFundName()) ); 
		return listInvestment;
		}
		
		// if no investment is done by user 
		else throw new NoInvestment("no investment using pan "+pan);
	}
	...
	public Investment summaryByFundIdService(String pan, long fundId) throws PanOrFundIdInvalid  {
		...
		if(investment.getPan().equalsIgnoreCase(pan)) return investment;
		
		// if there is no investment using given fundId or wrong pan is given
		else throw new PanOrFundIdInvalid("please provide valid pan and fundId");
	}
